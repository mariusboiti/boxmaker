...
function App() {
  ...
  const [settings, setSettings] = useState<BoxSettings>(initialDecodedSettings ?? DEFAULT_SETTINGS)
  ...
  const effectiveSettings = uiMode === 'simple' ? applySimpleDefaults(settings) : settings
  ...
  useEffect(() => {
    const handle = setTimeout(() => {
      try {
        const effectiveSettings = uiMode === 'simple' ? applySimpleDefaults(settings) : settings
        const result = generateBoxGeometry(effectiveSettings)
  ...
  }, [settings, uiMode, urlConfigWarning, importedItems, importWarnings])
                ...
                {selectedImportedItem ? (
                  <div className="mt-3 rounded-md border border-slate-800 bg-slate-950/40 p-2">
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-2">
                      <label className="flex items-center justify-between gap-2 text-[11px] text-slate-400">
                        ...
                        <input
                          type="number"
                          step={0.1}
                          value={Number.isFinite(selectedImportedItem.placement.x) ? selectedImportedItem.placement.x : 0}
                          onFocus={beginImportedItemsEdit}
                          onBlur={endImportedItemsEdit}
                          onChange={(e) => setSelectedPlacementLive({ x: Number(e.target.value) })}
                      ...
                      <label className="flex items-center justify-between gap-2 text-[11px] text-slate-400">
                        ...
                        <input
                          type="number"
                          step={0.1}
                          value={Number.isFinite(selectedImportedItem.placement.y) ? selectedImportedItem.placement.y : 0}
                          onFocus={beginImportedItemsEdit}
                          onBlur={endImportedItemsEdit}
                          onChange={(e) => setSelectedPlacementLive({ y: Number(e.target.value) })}
                          className="w-24 rounded-md border border-slate-700 bg-slate-900 px-2 py-1 text-[11px] text-slate-200"
                        />
                      </label>
                      <label className="flex items-center justify-between gap-2 text-[11px] text-slate-400">
                    ...
                    <div className="mt-2 text-[11px] text-slate-500">
                      Arrow keys move selected import. Shift = 10mm, Ctrl = 0.1mm.
                    </div>
                  </div>
                ) : null}
              </div>
