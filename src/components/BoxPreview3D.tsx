...
export const BoxPreview3D: FC<BoxPreview3DProps> = ({ settings, faces, importedItems }) => {
  ...
  const extrudedFaces = useMemo(() => {
    ...
    return relevant
      .map((face) => {
        ...
        const transform =
          face.name === 'lid'
            ? { position: [0, HB / 2 + t / 2, 0] as const, rotation: [-Math.PI / 2, 0, 0] as const }
  ...
  }, [D, HB, H, W, errorMessage, faces, hideLid, t, tMm])
